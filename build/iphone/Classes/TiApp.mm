...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kSplitViewSampleWithCommonJSUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kSplitViewSampleWithCommonJSUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}

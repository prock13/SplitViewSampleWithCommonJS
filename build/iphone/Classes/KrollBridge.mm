...
NSString * SplitViewSampleWithCommonJS$ModuleRequireFormat = @"(function(exports){"
		"var __OXP=exports;var module={'exports':exports};%@;\n"
		"if(module.exports !== __OXP){return module.exports;}"
		"return exports;})({})";


@implementation SplitViewSampleWithCommonJSObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_splitviewsamplewithcommonjs);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
-(void)injectPatches
{
	// called to inject any SplitViewSampleWithCommonJS patches in JS before a context is loaded... nice for 
...
{
	[context gc];
	[_splitviewsamplewithcommonjs gc];
}
...
{
	// create SplitViewSampleWithCommonJS global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_splitviewsamplewithcommonjs = [[SplitViewSampleWithCommonJSObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_splitviewsamplewithcommonjs];
	
	NSString *_splitviewsamplewithcommonjsNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _splitviewsamplewithcommonjsNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_splitviewsamplewithcommonjs valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:SplitViewSampleWithCommonJS$ModuleRequireFormat,code];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	if (data!=nil)
	{
		...
		if (![module respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			@throw [NSException exceptionWithName:@"org.splitviewsamplewithcommonjs.kroll" reason:[NSString stringWithFormat:@"Module \"%@\" failed to leave a valid exports object",path] userInfo:nil];
		}
	...
	@throw [NSException exceptionWithName:@"org.splitviewsamplewithcommonjs.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
